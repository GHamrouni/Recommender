...
int RMSE(learned_factors_t* learned,learning_model_t model,training_set_t * _tset_test){
	unsigned int i,j,l;
	double k;
	float sum=0;
	int s;
...
int main(void) {
	...
	params.training_set_size = 80000;
	...
	while (!feof(file)) {
		...
		set_known_rating(i-1,j-1,(float)k,tset_test);
	}
